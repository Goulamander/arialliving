...
(function (factory) {
  ...
  function closeSummerModal(){
    $('#summer-modal').modal('hide');
  }

  var dialog = renderer.create('<div id="summer-modal" class="modal" aria-hidden="false" tabindex="-1"/>', function ($node, options) {
    if (options.fade) {
      $node.addClass('fade');
    }
    $node.html([
      ...
      '      <button type="button" class="close" aria-label="Close" onClick="closeSummerModal()"><span aria-hidden="true">&times;</span></button>' +
